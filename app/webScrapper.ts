...
const onlyInYourStateURL: string = "https://www.onlyinyourstate.com/states/minnesota/";
const links: Link[] = [];

const promises: Promise<AxiosResponse<any>>[] = [axios.get(onlyInYourStateURL)];
...
for (let i: number = 2; i <= 30; i++) {
  promises.push(axios.get(`${onlyInYourStateURL}page/${i}/`));
}
    ...
    successfulPromises.map(page => {
        ...
        if (textArray.length > 1) {
          ...
          if (url.split("/minnesota/").length > 1) {
            ...
            name = names.map(thing =>thing.charAt(0).toUpperCase() + thing.slice(1)).join(" ");
            console.log(name);
            finalArray.push({
              address,
              name,
